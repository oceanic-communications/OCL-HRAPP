...
<div class="space-y-8">
    ...
    @if ($programmes === [])
        <div class="portal-card p-5">
            <p class="text-sm text-muted-foreground">No published active induction policies.</p>
        </div>
    @else
        @foreach ($programmes as $programme)
            @php
                ...
                $completions = $programme['completions'] ?? collect();
            ...
            <div class="space-y-4">
                <div class="portal-card space-y-4 p-5">
                    <h2 class="font-heading text-lg font-semibold text-foreground">{{ $version->policy->name }} ({{ $version->policy->abbreviation }})</h2>
                ...
                <div class="portal-card space-y-4 p-5">
                    <h3 class="font-heading text-base font-semibold text-foreground">Section acknowledgements</h3>

                    @if ($completions->isEmpty())
                        <p class="text-sm text-muted-foreground">
                            @if ($enrollment)
                                No clauses have been acknowledged yet for this policy.
                            @else
                                This employee has not started induction for this policy.
                            @endif
                        </p>
                    @else
